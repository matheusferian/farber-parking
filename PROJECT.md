...
**Cache version history:** `airvalet-shell-v1` (Phase 1, 2026-08-24) → **`airvalet-shell-v2`** (2026-08-24, this release) — bumped because Phase 2 changed `index.html`/`offline-db.js` substantially (real offline New Entry, IndexedDB v2) and the TV Mode/Quick H19 work changed `index.html`/`styles.css` again; a service worker cannot stay byte-identical to `v1` and still serve either of those changes to a device that already has `v1` installed and active. `SHELL_FILES` itself is unchanged — the same 8 files, verified by grepping every local `src`/`href` in `index.html` for anything new (nothing was added). The v1→v2 upgrade path was simulated end-to-end against a genuinely separate origin state (v1 installed and active, `airvalet_offline` at IndexedDB v1, real `deviceId`/`deviceCode='M1'`/offline-auth marker set, one diagnostic `offline_checkins` record) before this was considered safe to ship: v2 installs in the background while v1 keeps serving the active page (confirmed via `document.scripts`/function-presence checks — the page kept running v1 code, not v2, until explicit acceptance) → the existing "Update Available — Reload Now" banner appears (unchanged logic, only reached because a genuine update — not a first install — was detected) → `offlineApplyUpdate()` → `skipWaiting()` → single `controllerchange` reload → v2 active, `airvalet-shell-v1` cache actually removed (confirmed via `caches.keys()`) → IndexedDB auto-upgraded to v2 (`onupgradeneeded` added `server_snapshot` without touching `offline_checkins`) → the diagnostic record, `deviceId`, `deviceCode`, and the offline-auth marker all read back byte-identical to what was set before the upgrade → a cold launch with the dev server stopped entirely (genuinely no network path) loaded the full v2 application from cache with zero console errors.
