-- Rollback for 20260901_restrict_tv_only_role.sql.
-- Restores every policy and function touched by that migration to its
-- exact original form (captured via pg_policies/pg_get_functiondef()
-- before any change was made). Does not touch user_profiles/
-- current_user_role()/set_user_role() — run this before
-- 20260901_create_user_profiles_and_roles_rollback.sql if rolling back
-- both, since these policies were defined in terms of current_user_role().

-- ── passengers ───────────────────────────────────────────────────────
drop policy if exists "authenticated_passengers_select" on public.passengers;
create policy "authenticated_passengers_select" on public.passengers for select to authenticated using (true);

drop policy if exists "authenticated_passengers_insert" on public.passengers;
create policy "authenticated_passengers_insert" on public.passengers for insert to authenticated with check (true);

drop policy if exists "authenticated_passengers_update" on public.passengers;
create policy "authenticated_passengers_update" on public.passengers for update to authenticated using (true) with check (true);

drop policy if exists "authenticated_passengers_delete" on public.passengers;
create policy "authenticated_passengers_delete" on public.passengers for delete to authenticated using (true);

-- ── activity_log ─────────────────────────────────────────────────────
drop policy if exists "authenticated_logs_select" on public.activity_log;
create policy "authenticated_logs_select" on public.activity_log for select to authenticated using (true);

drop policy if exists "authenticated_logs_insert" on public.activity_log;
create policy "authenticated_logs_insert" on public.activity_log for insert to authenticated with check (true);

-- ── contact_sync_batches / contact_sync_items ───────────────────────
drop policy if exists "csb_select_authenticated" on public.contact_sync_batches;
create policy "csb_select_authenticated" on public.contact_sync_batches for select to authenticated using (true);

drop policy if exists "csb_insert_own" on public.contact_sync_batches;
create policy "csb_insert_own" on public.contact_sync_batches for insert to authenticated
  with check (created_by = (auth.jwt() ->> 'email'::text));

drop policy if exists "csb_update_authenticated" on public.contact_sync_batches;
create policy "csb_update_authenticated" on public.contact_sync_batches for update to authenticated using (true) with check (true);

drop policy if exists "csi_select_authenticated" on public.contact_sync_items;
create policy "csi_select_authenticated" on public.contact_sync_items for select to authenticated using (true);

drop policy if exists "csi_insert_into_own_generated_batch" on public.contact_sync_items;
create policy "csi_insert_into_own_generated_batch" on public.contact_sync_items for insert to authenticated
...
create policy "dcr_select_authenticated" on public.daily_closing_reports for select to authenticated using (true);

drop policy if exists "dcr_insert_authenticated" on public.daily_closing_reports;
create policy "dcr_insert_authenticated" on public.daily_closing_reports for insert to authenticated with check (true);

drop policy if exists "dcr_update_authenticated" on public.daily_closing_reports;
create policy "dcr_update_authenticated" on public.daily_closing_reports for update to authenticated using (true) with check (true);

-- ── passenger_return_date_changes ────────────────────────────────────
drop policy if exists "prdc_select_authenticated" on public.passenger_return_date_changes;
create policy "prdc_select_authenticated" on public.passenger_return_date_changes for select to authenticated using (true);

-- ── SECURITY DEFINER write functions — restored to their exact
-- pre-Phase-5 bodies (no role guard) ─────────────────────────────────
