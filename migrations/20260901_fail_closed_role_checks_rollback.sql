-- Rollback for 20260901_fail_closed_role_checks.sql.
-- Restores every policy's role clause and both functions' guard line to
-- their exact prior live text (the "IS DISTINCT FROM 'TV_ONLY'" /
-- "= 'TV_ONLY'" fail-open forms) — same restore-prior-live-state
-- convention as every other rollback in this phase, even though that
-- prior state is the bug this migration fixes. All non-role conditions
-- (ownership checks, exists(...) subqueries) are unchanged either way.
...
-- ── SECURITY DEFINER write functions — guard line restored ──────────
